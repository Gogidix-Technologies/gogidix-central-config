...
# Development utility script for secrets-management
...
start_service() {
    echo "Starting $secrets-management..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${secrets-management}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$secrets-management started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $secrets-management..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$secrets-management stopped"
        else
            echo "$secrets-management is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $secrets-management may not be running"
    fi
...
run_tests() {
    echo "Running tests for $secrets-management..."
    cd "$SERVICE_DIR"
...
build_service() {
    echo "Building $secrets-management..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $secrets-management in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
