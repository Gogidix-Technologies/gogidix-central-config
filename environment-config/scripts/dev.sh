...
# Development utility script for environment-config
...
start_service() {
    ...
    nohup java -jar target/${environment-config}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$environment-config started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $environment-config..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$environment-config stopped"
        else
            echo "$environment-config is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $environment-config may not be running"
    fi
...
run_tests() {
    echo "Running tests for $environment-config..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $environment-config in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $environment-config health..."
