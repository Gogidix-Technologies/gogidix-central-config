...
# Development utility script for ci-cd-templates
...
start_service() {
    ...
    nohup java -jar target/${ci-cd-templates}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$ci-cd-templates started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $ci-cd-templates..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$ci-cd-templates stopped"
        else
            echo "$ci-cd-templates is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $ci-cd-templates may not be running"
    fi
...
run_tests() {
    echo "Running tests for $ci-cd-templates..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $ci-cd-templates in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $ci-cd-templates health..."
