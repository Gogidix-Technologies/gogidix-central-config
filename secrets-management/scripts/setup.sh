...
echo "Setting up secrets-management development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/secrets-management_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/secrets-management_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/secrets-management_db
DATABASE_USERNAME=secrets-management_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U secrets-management_user -d secrets-management_db < database/migrations/init.sql
fi
