...
# Setup script for environment-config
# This script sets up the development environment for the service

set -e

echo "Setting up environment-config development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/environment-config_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/environment-config_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/environment-config_db
DATABASE_USERNAME=environment-config_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U environment-config_user -d environment-config_db < database/migrations/init.sql
fi
