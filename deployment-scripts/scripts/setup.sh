...
# Setup script for deployment-scripts
# This script sets up the development environment for the service

set -e

echo "Setting up deployment-scripts development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/deployment-scripts_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/deployment-scripts_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/deployment-scripts_db
DATABASE_USERNAME=deployment-scripts_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U deployment-scripts_user -d deployment-scripts_db < database/migrations/init.sql
fi
