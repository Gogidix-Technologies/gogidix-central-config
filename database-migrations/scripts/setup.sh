...
echo "Setting up database-migrations development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/database-migrations_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/database-migrations_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/database-migrations_db
DATABASE_USERNAME=database-migrations_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U database-migrations_user -d database-migrations_db < database/migrations/init.sql
fi
