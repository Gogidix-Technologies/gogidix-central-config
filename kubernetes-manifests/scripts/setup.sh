...
# Setup script for kubernetes-manifests
# This script sets up the development environment for the service

set -e

echo "Setting up kubernetes-manifests development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/kubernetes-manifests_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/kubernetes-manifests_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/kubernetes-manifests_db
DATABASE_USERNAME=kubernetes-manifests_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U kubernetes-manifests_user -d kubernetes-manifests_db < database/migrations/init.sql
fi
