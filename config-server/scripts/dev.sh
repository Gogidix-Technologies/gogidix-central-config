...
# Development utility script for config-server
...
start_service() {
    ...
    nohup java -jar target/${config-server}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$config-server started with PID $(cat .pid)"
...
stop_service() {
    ...
    if [ -f .pid ]; then
        ...
        echo "PID file not found. $config-server may not be running"
    fi
...
dev_mode() {
    echo "Starting $config-server in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
