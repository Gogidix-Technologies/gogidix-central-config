...
# Setup script for infrastructure-as-code
# This script sets up the development environment for the service

set -e

echo "Setting up infrastructure-as-code development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/infrastructure-as-code_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/infrastructure-as-code_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/infrastructure-as-code_db
DATABASE_USERNAME=infrastructure-as-code_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U infrastructure-as-code_user -d infrastructure-as-code_db < database/migrations/init.sql
fi
