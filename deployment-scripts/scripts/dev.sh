...
# Development utility script for deployment-scripts
...
start_service() {
    ...
    nohup java -jar target/${deployment-scripts}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$deployment-scripts started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $deployment-scripts..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$deployment-scripts stopped"
        else
            echo "$deployment-scripts is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $deployment-scripts may not be running"
    fi
...
run_tests() {
    echo "Running tests for $deployment-scripts..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $deployment-scripts in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $deployment-scripts health..."
