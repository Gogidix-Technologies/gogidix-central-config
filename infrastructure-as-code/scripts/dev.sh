...
# Development utility script for infrastructure-as-code
...
start_service() {
    echo "Starting $infrastructure-as-code..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${infrastructure-as-code}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$infrastructure-as-code started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $infrastructure-as-code..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$infrastructure-as-code stopped"
        else
            echo "$infrastructure-as-code is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $infrastructure-as-code may not be running"
    fi
...
run_tests() {
    echo "Running tests for $infrastructure-as-code..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $infrastructure-as-code in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $infrastructure-as-code health..."
...
service_info() {
    ...
    echo "==================="
    echo "Name: $infrastructure-as-code"
