...
# Development utility script for database-migrations
...
start_service() {
    ...
    nohup java -jar target/${database-migrations}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$database-migrations started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $database-migrations..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$database-migrations stopped"
        else
            echo "$database-migrations is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $database-migrations may not be running"
    fi
...
run_tests() {
    echo "Running tests for $database-migrations..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $database-migrations in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $database-migrations health..."
