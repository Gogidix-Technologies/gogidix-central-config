...
# Setup script for config-server
# This script sets up the development environment for the service

set -e

echo "Setting up config-server development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/config-server_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/config-server_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/config-server_db
DATABASE_USERNAME=config-server_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U config-server_user -d config-server_db < database/migrations/init.sql
fi
