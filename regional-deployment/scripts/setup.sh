...
# Setup script for regional-deployment
# This script sets up the development environment for the service

set -e

echo "Setting up regional-deployment development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/regional-deployment_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/regional-deployment_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/regional-deployment_db
DATABASE_USERNAME=regional-deployment_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U regional-deployment_user -d regional-deployment_db < database/migrations/init.sql
fi
