...
# Development utility script for kubernetes-manifests
...
start_service() {
    echo "Starting $kubernetes-manifests..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${kubernetes-manifests}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$kubernetes-manifests started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $kubernetes-manifests..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$kubernetes-manifests stopped"
        else
            echo "$kubernetes-manifests is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $kubernetes-manifests may not be running"
    fi
...
run_tests() {
    echo "Running tests for $kubernetes-manifests..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $kubernetes-manifests in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $kubernetes-manifests health..."
...
service_info() {
    ...
    echo "==================="
    echo "Name: $kubernetes-manifests"
