...
# Setup script for ci-cd-templates
# This script sets up the development environment for the service

set -e

echo "Setting up ci-cd-templates development environment..."
...
mkdir -p src/main/java/com/exalt/centralconfig/ci-cd-templates_CLEAN
mkdir -p src/main/resources
mkdir -p src/test/java/com/exalt/centralconfig/ci-cd-templates_CLEAN
...
DATABASE_URL=jdbc:postgresql://localhost:5432/ci-cd-templates_db
DATABASE_USERNAME=ci-cd-templates_user
...
if [ -f "database/migrations/init.sql" ]; then
    docker exec -i $(docker-compose ps -q postgres) psql -U ci-cd-templates_user -d ci-cd-templates_db < database/migrations/init.sql
fi
