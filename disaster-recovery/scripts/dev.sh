...
# Development utility script for disaster-recovery
...
start_service() {
    ...
    nohup java -jar target/${disaster-recovery}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$disaster-recovery started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $disaster-recovery..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$disaster-recovery stopped"
        else
            echo "$disaster-recovery is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $disaster-recovery may not be running"
    fi
...
run_tests() {
    echo "Running tests for $disaster-recovery..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $disaster-recovery in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $disaster-recovery health..."
